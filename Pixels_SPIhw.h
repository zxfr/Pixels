...
protected:
    ...
    void writeDataBytes(uint8_t * data, uint32_t size);
    void writeDataPattern(uint8_t * data,uint8_t size, uint32_t repeat);
...
#if defined(ESP8266)
void SPIhw::writeData16(uint16_t data) {
  if (eightBit) {
      sbi(GPOS,bitmaskWR);
    }
    if (hwCS){
      SPI.write16(data, true);
    } else {
       cbi(GPOC,bitmaskCS); //cs low 
       SPI.write16(data, true);
       sbi(GPOS,bitmaskCS); //cs high
...
#if defined(ESP8266)
void SPIhw::writeDataBytes(uint8_t * data, uint32_t size) {
  if (eightBit) {
        sbi(GPOS,bitmaskWR);
      }
      if (hwCS){
...
#if defined(ESP8266)
void SPIhw::writeDataPattern(uint8_t * data, uint8_t size, uint32_t repeat) {
   if (eightBit) {
        sbi(GPOS,bitmaskWR);
      }
      if (hwCS){
